...
\begin{figure}[htbp]
    \centering
    \includegraphics[scale=0.17]{images/diagram1.png}
    \caption{Block diagram of the signal processing and analysis system}
...
The proposed system is designed to perform robust heart rate variability analysis using consumer-grade mobile devices. The data processing pipeline, illustrated in Figure~\ref{fig:system_diagram}, consists of two parallel paths for ECG and PPG signal analysis.
%\ref{fig:system_diagram}
%TODO dopisac o diagramie, wkleic diagram, dodac opis

\begin{figure}[htbp]
    \centering
    % Zmieniono szerokość na 0.6\textwidth (60% szerokości strony)
    ...
    \begin{tikzpicture}[
        ...
        % PPG Path
        \node [block, below left=1cm and 0.5cm of input] (camera) {Smartphone Camera (PPG)};
        ...
        % ECG Path
        \node [block, below right=1cm and 0.5cm of input] (polar) {Polar H10 Sensor (ECG)};
        ...
        % Merge
        \node [block, below right=1cm and -0.5cm of cnn] (peaks) {Peak Timestamp Extraction};
